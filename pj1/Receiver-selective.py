...
class Connection():
    ...
    def ack(self,seqno, data):
        ...
        return self.current_seqno+1, res_data,self.seq_list
...
class Receiver():
    ...
    def _send_ack(self, seqno, address,seqno_list):
        m = "ack|%d|" % seqno
        i=len(seqno_list)
        if (i>0):
            s=str(seqno_list[0])
    ...
    def _handle_start(self, seqno, data, address):
        ...
        ackno, res_data,seq_list = conn.ack(seqno,data)
        for l in res_data:
            if self.debug:
                print data
            conn.record(l)
        self._send_ack(ackno, address,seq_list)
